...
Drift::Drift(const InstanceInfo& info)
...
void Drift::OnParamChange(int paramIdx)
{
    for (int i = 0; i < 8; ++i)
    {
...
bool Drift::OnMessage(int msgTag, int ctrlTag, int dataSize, const void* pData)
{
...
// static
void Drift::ReseedTrack(EuclideanTrack& t)
{
...
void Drift::KillExistingNote(int noteNumber, int channel)
{
...
void Drift::ProcessStep(int absoluteStep, double bpm, double sampleRate)
{
...
WDL_String Drift::ExportPatternAsMidiFile()
{
    ...
    for (int v = 0; v < variations; ++v)
    {
        ...
        WDL_String path;
        if (variations == 1)
            path.SetFormatted(1024, "%s/Drift_Export.mid", tmpDir);
        else
            path.SetFormatted(1024, "%s/Drift_Export_%d.mid", tmpDir, v + 1);
